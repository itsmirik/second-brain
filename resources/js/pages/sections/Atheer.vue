...
const generatedAt = computed(() => {
    ...
    return new Date(props.report.generated_at).toLocaleString('en-GB');
});
...
<template>
    ...
    <AppLayout title="Atheer">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
                    <span v-if="generatedAt">As of {{ generatedAt }}</span>
                    <span v-else>Live business figures.</span>
                </p>
            </div>
            <button
                type="button"
        ...
        >
            {{ error }} The figures below could not be loaded — try Refresh.
        </div>
        ...
        <template v-if="report">
            ...
            <div v-if="deliveries" class="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
                <StatCard label="In transit" :value="count(deliveries.in_transit)" />
                <StatCard label="Delivered today" :value="count(deliveries.delivered_today)" tone="positive" />
                <StatCard label="Flagged" :value="count(deliveries.flagged)" :tone="deliveries.flagged ? 'warning' : 'default'" />
                <StatCard label="Flagged amount" :value="money(deliveries.flagged_amount)" :tone="deliveries.flagged_amount ? 'warning' : 'default'" />
            </div>
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-neutral-50 text-left text-xs text-neutral-500 uppercase dark:bg-neutral-900 dark:text-neutral-400">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-right font-medium">COD</th>
                            <th class="px-4 py-2 font-medium">Dispatched</th>
                        </tr>
                    </thead>
            ...
            <h3 class="mb-3 text-sm font-semibold text-neutral-500 dark:text-neutral-400">
                ...
                <span v-if="reconciliation" class="font-normal text-neutral-400">
                    ({{ count(reconciliation.flagged_count) }} flagged · {{ money(reconciliation.flagged_amount) }})
                </span>
            </h3>
            <div
            ...
            >
                Nothing to chase — all deliveries reconciled.
            </p>
